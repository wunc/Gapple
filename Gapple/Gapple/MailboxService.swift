...
class MailboxService {
    ...
    func selectSpecialMailbox(_ sender: NSObject, zelector: Zelector) -> Void {
        sender.eventDate = nil;
        if sender.className != Clazz.SingleMessageViewer.rawValue {
            sender.perform(Zelector.setSelectedMailboxes.get(), with: [specialMailbox(sender, zelector: zelector)])
        }
    }
    
    func toMailbox(_ sender:NSObject, toAction: ToAction) {
        sender.eventDate = nil;
        if sender.className != Clazz.SingleMessageViewer.rawValue {
            ...
            var keys = [String]()
            let sortedKeys = Array(mailboxDict.keys).sorted(by: <)
            for key in sortedKeys {
                keys.append(String(key))
            }
            
            var tvc: ToViewController!
            let gappleBundle = Bundle(identifier: "ironfish.Gapple")
            tvc = ToViewController(nibName: "ToViewController", bundle: gappleBundle)
            tvc.setupPaths(_paths: keys)
            tvc.toAlert(messageText: messageText, sender: sender, selector: selector, dict: mailboxDict)
        }
    }
    ...
    private func copyMailboxesDict(_ sender:NSObject) -> [String:MailboxEntry]{
        let mailboxes = accountMailboxesDict(ToAction.Copy)
        return mailboxes
    }
    
    func goMailboxesDict(_ sender:NSObject) -> [String:MailboxEntry]{
    ...
    private func archiveMailbox(_ sender:AnyObject) -> AnyObject {
        let dict:[String:AnyObject] = favoritesBarMailboxes(sender)
        return dict["\(Zelector.Archive)"]!
    }
    
    private func flaggedMailbox(_ sender:AnyObject) -> AnyObject {
        let dict:[String:AnyObject] = favoritesBarMailboxes(sender)
        return dict["\(Zelector.Flagged)"]!
    }
    
    private func moveMailboxesDict(_ sender:NSObject) -> [String:MailboxEntry]{
        let mailboxes = accountMailboxesDict(ToAction.Move)
        return mailboxes
    }
    
    private func specialMailbox(_ sender:NSObject, zelector: Zelector) -> AnyObject {
    ...
    private func smartMailboxesDict() -> [String:MailboxEntry] {
        var dict = [String:MailboxEntry]()
        let mailboxController: AnyObject = utils.getUnretainedObject(NSApplication.shared, selector: Zelector.mailboxesController.get())
        let smartMailboxes = utils.getUnretainedObjectAsArray(mailboxController, selector: Zelector.smartMailboxes.get())
        
        func _toMailboxDict(_ mailboxes:[AnyObject], parentPath: String) -> Void {
    ...
    private func favoritesBarMailboxes(_ sender:AnyObject) -> [String:AnyObject] {
    ...
    private func accountMailboxesDict(_ toAction: ToAction) -> [String:MailboxEntry]{
        var accountMailboxes = [String:MailboxEntry]()
        let accounts = utils.getUnretainedObjectAsArray(NSApplication.shared, selector: Zelector.accounts.get())
    ...
    private func validMoveToMailbox(_ name: String) -> Bool {
        let names = ["All Mail", "Archive", "Drafts", "Sent Mail", "Sent Messages"]
        return names.contains(name) ? false : true
    }
    
    private func validCopyToMailbox(_ name: String) -> Bool {
